...
import os
import json
from training.model import VisionTransformer, Qwen2_5_Custom_Vit
from qwen2_5_model_base import Qwen2_5_VLProcessor
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, 
    ...
    if 'lora' in model_name.lower() and model_base is not None:
        ...
        elif "custom" in model_base:
            processor = Qwen2_5_VLProcessor.from_pretrained(model_base)
            
            model = Qwen2_5_Custom_Vit.from_pretrained(model_base, low_cpu_mem_usage=True, config=lora_cfg_pretrained, **kwargs)
        ...
        elif "qwen2_5" in config["_name_or_path"]:
            processor = AutoProcessor.from_pretrained(model_path)
            model = Qwen2_5_VLForConditionalGeneration.from_pretrained(model_path, low_cpu_mem_usage=True, **kwargs)
...
def merge_vit_qwen2_5_weights(model_base):
    kwargs = {"device_map": "cpu"}
    
    cfg_pretrained = AutoConfig.from_pretrained(model_base) # model_path
    cfg_pretrained.architectures = ["Qwen2_5_Custom_Vit"]
    
    processor = Qwen2_5_VLProcessor.from_pretrained(model_base)
    print('Loading Qwen2.5-VL from base model...')
    model = Qwen2_5_VLForConditionalGeneration.from_pretrained(model_base, low_cpu_mem_usage=True, config=cfg_pretrained, **kwargs)
    token_num, tokem_dim = model.lm_head.out_features, model.lm_head.in_features
    if model.lm_head.weight.shape[0] != token_num:
        model.lm_head.weight = torch.nn.Parameter(torch.empty(token_num, tokem_dim, device=model.device, dtype=model.dtype))
        model.model.embed_tokens.weight = torch.nn.Parameter(torch.empty(token_num, tokem_dim, device=model.device, dtype=model.dtype))
    
    print('Loading vision transformer weights...')    
    visual = VisionTransformer("vit_large_patch16_224", cfg_pretrained.vision_config, load_pretrained=True)
    model.visual = visual

    print('Merge Model Loaded!!!')
    return processor, model
